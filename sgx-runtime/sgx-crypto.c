...
#include <assert.h> // harpuia: debug
//#define SGX_CRYPTO_DEBUG
#undef NDEBUG
...
void measure_chunk_page(void *measurement, void *page, uint64_t chunk_offset)
{
...
#if SGX_CRYPTO_DEBUG
    {
        char hash[64+1];
        fmt_hash(measurement, hash);
        sgx_dbg(info, "virtual extend: %.20s.., counter: %ld", hash,
                                                                g_update_counter);
    }
#endif
                      ...
                      uint64_t page_offset)
{
    ...
    memset (tmp_update_field, 0, sizeof (uint64_t) * 8);
...
#if SGX_CRYPTO_DEBUG
    {
        char hash[64+1];
        fmt_hash(measurement, hash);
        sgx_dbg(info, "pre-extend add: %.20s.., counter: %ld", hash,
                                                               g_update_counter);
	fputs ("Page being added: \n", stderr);
	hexdump(stderr, page, 400);
    }
#endif
...
void set_entry(tcs_t *tcs, size_t offset)
{
    ...
    tcs->oentry = offset;
    printf("tls_npages is %d, tcs oentry is %p\n", tls_npages, tcs->oentry); //cch added for debug
}
...
void generate_enclavehash(void *hash, void *code, int code_pages,
                          size_t entry_offset)
{
    // FIXME: I cannot handle multithreaded TCS!
    /* harpuia: Current measurement generation order
     * - Zero-fill and initialize
     * - Measure ECREATE
     * {
     *     - Measure TCS
     *     - Measure SSA (empty)
     ...
     * Measure code
     * Measure heap (empty)
     * Measure stack (empty) for each TCS
     * EINIT
     */
    tcs_t *tmp_tcs;
    ...
    uint64_t enclave_size;
    void *page;
    char *empty_page; //cch added
    uint64_t stack_begin; // harpuia added

    // FIXME! Entry offset might be defined separately among threads later!
    size_t real_entry_offset[NUMBER_OF_THREADS];
    for (int i = 0; i < NUMBER_OF_THREADS; i++)
        real_entry_offset[i] = entry_offset;
#ifdef THREAD_ATTACK
    real_entry_offset[1] = 0x10c;
#endif


    // cch: these lines are moved from sgx-kern.c
    ...
    int ssa_npages = SSA_PAGE_FRAMES;
    int stack_npages = STACK_PAGE_FRAMES_PER_THREAD;
    int heap_npages = HEAP_PAGE_FRAMES;

    int npages = sec_npages + code_pages + (tcs_npages + tls_npages +
                                            ssa_npages + stack_npages)
                                           * NUMBER_OF_THREADS
                                         + heap_npages;
    ...
    int tcs_page_offset[NUMBER_OF_THREADS];
    int tls_page_offset[NUMBER_OF_THREADS];
    int ssa_page_offset[NUMBER_OF_THREADS];
    for (int i = 0; i < NUMBER_OF_THREADS; i++)
    {
        if (i == 0) {
	...
	else
	{
	    ...
	    ssa_page_offset[i] = ssa_page_offset[i-1] + tcs_npages + ssa_npages + tls_npages;
	    tls_page_offset[i] = tls_page_offset[i-1] + tcs_npages + ssa_npages + tls_npages;
	}
    }
    int code_page_offset = tls_page_offset[NUMBER_OF_THREADS - 1] + tls_npages;
    int stack_page_offset[NUMBER_OF_THREADS];
    for (int i = 0; i < NUMBER_OF_THREADS; i++)
	if (i == 0)
	    stack_page_offset[i] = code_page_offset + code_pages + stack_npages;
	else
	    stack_page_offset[i] = stack_page_offset[i-1] + stack_npages;
    int heap_page_offset = stack_page_offset[NUMBER_OF_THREADS - 1];

    for (int i = 0; i < NUMBER_OF_THREADS; i++)
    {
        memset (tmp_tcs, 0, PAGE_SIZE);
	set_tcs_fields(tmp_tcs, real_entry_offset[i]);
	// Initialize secinfo.
	...
	// Actually measure the TCS.
	update_tcs_fields(tmp_tcs, tls_page_offset[i], ssa_page_offset[i], code_page_offset);
	memcpy (&current_page, tmp_tcs, PAGE_SIZE);
	measure_page_add (hash, &current_page, &tmp_secinfo, tcs_page_offset[i] * 0x1000);
	...
	// harpuia: Correct order is SSA BEFORE TLS
	// Measrue ssa pages.
	page = (void *)empty_page;
	for (int j = 0; j < ssa_npages; j++) {
	    memset(&current_page, 0, PAGE_SIZE);
	    measure_page_add(hash, &current_page, &tmp_secinfo, (ssa_page_offset[i] + j) * 0x1000);
	}

	// Measure tls pages.
	page = (void *)empty_page;
	// enclave_addr = 0x50000000 - 0x1000 * code_page_offset
	// stack_linaddr = enclave_addr + stack_page_offset[i] * 0x1000
	//               = 0x50000000 + 0x1000 * (stack_page_offset[i] - code_page_offset)
	// Therefore, stack_begin = stack_linaddr - 0x1000 + 0xff0
	//                        = 0x4ffffff0 + 0x1000 * (stack_page_offset[i] - code_page_offset)
	stack_begin = 0x4ffffff0 + 0x1000 * (stack_page_offset[i] - code_page_offset);
	
	for (int j = 0; j < tls_npages; j++) {
	    memset(&current_page, 0, PAGE_SIZE);
	    memcpy (&current_page, &stack_begin, sizeof (uint64_t)); // harpuia: TLS should have this address
	    measure_page_add (hash, &current_page, &tmp_secinfo, (tls_page_offset[i] + j) * 0x1000);
	}
    ...
    for (int i = 0; i < code_pages; i++) {
        ...
        measure_page_add(hash, &current_page, &tmp_secinfo, (code_page_offset + i) * 0x1000);
    ...
    for (int i = 0; i < heap_npages; i++) {
        memset(&current_page, 0, PAGE_SIZE);
        //memcpy(&current_page, &page, sizeof(uintptr_t)); // harpuia: see 366
        measure_page_add(hash, &current_page, &tmp_secinfo, (heap_page_offset + i) * 0x1000);
    }

    // Measure stack pages.
    // harpuia: Stack is allocated in inverse order.
    page = (void *)empty_page;
    for (int j = 0; j < NUMBER_OF_THREADS; j++) {
	for (int i = 0; i < stack_npages; i++) {
	    memset(&current_page, 0, PAGE_SIZE);
	    measure_page_add(hash, &current_page, &tmp_secinfo, (stack_page_offset[j] - i - 1) * 0x1000);
	}
    }
