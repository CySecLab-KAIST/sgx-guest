...
#include <stdint.h> //cch: for int8_t family
...
extern int sgx_make_quote(const char* pers, report_t *report,
        unsigned char *rsa_N, unsigned char *rsa_E);
// END sgx-attest.c

// BEGIN sgx-attest-local.c
extern int sgx_local_attest_challenger(int target_port, char *conf);
extern int sgx_local_attest_target(int challenger_port);
// END sgx-attest-local.c

// BEGIN sgx-attest-remote.c
